	...
	def base_info
	  ...
	  def get_c_type
		...
		def get_end_time
			...
			date =  Date.parse(registrationDay)

			if end_sample != "至无固定期限"
				end_at = (date + end_sample.year).strftime("%Y-%m-%d")

				res = "#{for_time(registrationDay)}至#{end_at.split("-")[0]+"年"+end_at.split("-")[1]+"月"+end_at.split("-")[2]+"日"}"
				res

			else
				end_str =  "#{for_time(registrationDay)}至无固定期限"
			end
